...
pkl_file = open(
    "check-Disease.pkl", 'rb')
checkDiseaseModel = pickle.load(
    open("check-Disease.pkl", 'rb'))

# Load the preprocessed data
data = pd.read_csv("check-Disease.csv")

# Extract the input and output data
checkInput_data = data.drop(["Disease"], axis=1)
checkOutput_data = data["Disease"]
checkInput_data = pd.get_dummies(checkInput_data)
...
# check disease api call model runs here
...
def predict():
    if request.method == 'POST':
        ...
        new_data = pd.DataFrame({
            'Select Symptom 1': [symptom1],
            'Select Symptom 2': [symptom2],
            'Select Symptom 3': [symptom3],
            'Select Symptom 4': [symptom4],
            'Select Symptom 5': [symptom5],
            'Select Symptom 6': [symptom6]
        })
        new_data = pd.get_dummies(new_data).reindex(columns=checkInput_data.columns, fill_value=0)
        prediction = checkDiseaseModel.predict(new_data)

    # 4. render to template the predict function and
    # 5. storing Predict_score and will be used in template

    return render_template('check.html', Disease=prediction[0])
